...
class Debugger:
    ...
    def cmd_bp_set(self, rest: str) -> None:
        ...
        if self._try_set_symbol_bp(rest):
            ...
            self._breakpoints[-1].desc += (
                f" if {cond_type} {cond_reg}{cond_op}" + f"0x{cond_val:x}"
            )
    ...
    def _warn_pc_if_suspect(self, h, v: int) -> None:
        ...
        """
        if (
            h.mmu_mode == 0 and h._bus is not None and not h._bus.is_valid_addr(v)
        ):  # Bare — 物理地址
            self._warn(
    ...
    def cmd_pc(self, addr: str | None = None) -> None:
        ...
        if h.mode in set((RiscvMode.M, RiscvMode.D)):  # M / D 模式 — M 侧亮, S 侧 dim
            ...
            stale_note = "" if _scause != 0 else " [dim](M-mode 陈旧)[/]"
        else:  # U 模式
        ...
        m_anno = (
            f"[cyan]{_mcause_name}[/]  MIE={(_mstatus >> 3) & 1} MPP={(_mstatus >> 11) & 3}"
        )
        s_anno = ""
        if _scause != 0:
            _scause_name = self._trap_cause_name(_scause)
            s_anno = (
                f"[cyan]{_scause_name}[/]  SIE={(_sstatus >> 1) & 1} SPP={(_sstatus >> 8) & 1}"
            )
        elif _sstatus != 0:
            s_anno = f"SIE={(_sstatus >> 1) & 1} SPP={(_sstatus >> 8) & 1}"
    ...
    def _show_deleg(self, name: str, v: int, names: dict[int, str]) -> None:
        tbl = Table(title=f"{name} 内部字段 (置位 = 委派到 S 模式)", border_style="magenta")
        tbl.add_column("Field", style="cyan")
        tbl.add_column("Bit", style="yellow", justify="center")
    ...
    def cmd_pt(self, arg: str | None = None) -> None:
        ...
        if l1_raw is None:
            ...
            self._console.print("\n".join(out))
            return
        l1_pte = int.from_bytes(l1_raw[vpn2 * 8 : vpn2 * 8 + 8], "little", signed=False)
        ...
        if l2_raw is None:
            ...
            self._console.print("\n".join(out))
            return
        l2_pte = int.from_bytes(l2_raw[vpn1 * 8 : vpn1 * 8 + 8], "little", signed=False)
        ...
        if l3_raw is None:
            ...
            self._console.print("\n".join(out))
            return
        l3_pte = int.from_bytes(l3_raw[vpn0 * 8 : vpn0 * 8 + 8], "little", signed=False)
    ...
    def _pt_append_level(
        ...
        out.append(f"  L{level}[{idx:#05x}] = 0x{pte:016x}  PPN=0x{ppn:09x}  → {status}")
    ...
    def cmd_vdisasm(self, addr_str: str, inst_count_str: str = "16") -> None:
        ...
        while offset < max_offset and len(instrs) < inst_count:
            ...
            pc_pa = (
                first_pa + offset
            )  # 同页内 VA/PA 偏移一致; 跨页已由 _try_read_va_forced 处理
            remaining = max_offset - offset
    ...
    def _show_va_mapping_header(self, va: int, size: int) -> None:
        ...
        for pi in range(page_cnt):
            ...
            if not hit and h._mem_read_phy is None:
                lines.append(f"  [red]VA 0x{page_va:016x}  内存后端未挂载[/]")
                continue
                ...
                if ok:
                    continue
                lines.append(f"  [red]VA 0x{page_va:016x}  缺页 (无法翻译)[/]")
            ...
            lines.append(f"  VA 0x{page_va:016x} → PA 0x{pa:016x}  [{r}{w}{x}{u}]")
    ...
    def cmd_vmem(self, addr_str: str, size_str: str = "64") -> None:
        ...
        self._console.print(f"[dim]VA {self._hex(va)} → PA {self._hex(first_pa)}[/]")
    ...
    def _show_hart_detail(self, hart_id: int) -> None:
        ...
        instr_fmt = (
            self._fmt_instr_count(self._instr_count) if hart_id == self._hart_id else "0"
        )
        tbl = Table(
    ...
    def _parse_trap_save_offsets(
        self, tvec: int, max_instrs: int = 20
    ) -> tuple[int, int] | None:
    ...
    def _add_prev_mode_frame(
        ...
        boundary_mode, boundary_note = self._resolve_boundary_frame(trapped_pc, prev_name)
        ...
        search_base = (
            min(last_fp if last_fp else last_sp, last_sp if last_sp else last_fp) & ~0x7
        )
        if search_base == 0:
            # 没有已知栈指针 → 无法定位 trap 帧
            frames.append(
    ...
    def _walk_prev_mode_frames(
        ...
        self._add_prev_mode_frame(frames, trapped_pc, prev_mode, tvec, visited)
    ...
    def cmd_frame(self, arg: str | None = None) -> None:
        ...
        for f in self._stack_frames:
            ...
            if f.note:
                ...
                regs = f"pc={self._hex(f.pc)}  [bold magenta]{f.note}[/]"
