...
/// Public alias for use by the batch engine's inline memory access helpers.
#[inline(always)]
...
mod tests {
    ...
    fn test_read_ram_normal() {
        ...
        let mut out = [0u8; 8];
        let ok = bus_read_ram(
            ram.as_ptr(), 16, 0x8000_0000, 0, 0,
            0x8000_0004, 4, out.as_mut_ptr(),
        );
        assert_eq!(ok, 1);
    ...
    fn test_read_shadow() {
        ...
        let ok = bus_read_ram(
            ram.as_ptr(), 16, 0x8000_0000, 0x0, 16,
            0x0000_0008, 4, out.as_mut_ptr(),
        );
        assert_eq!(ok, 1);
    ...
    fn test_write_ram() {
        ...
        let ok = bus_write_ram(
            ram.as_mut_ptr(), 16, 0x8000_0000, 0, 0,
            0x8000_0008, data.as_ptr(), 4,
        );
        assert_eq!(ok, 1);
    ...
    fn test_read_sizes() {
        ...
        assert_eq!(ok, 1);
        assert_eq!(u32::from_le_bytes([out[0], out[1], out[2], out[3]]), 0x08040201);
