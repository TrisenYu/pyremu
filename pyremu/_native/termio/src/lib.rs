...
impl EscBuf {
	...
	fn feed(&mut self, b: u8) -> Option<&[u8]> {
		...
		if self.buf[1] == b'[' && (0x40..=0x7E).contains(&b) {
			return self.flush();
		}
...
fn termio_thread(h: TermIoHandle, saved: SavedTerm) {
	...
	while stop.load(Ordering::Acquire) == 0 {
		...
		let nfds: libc::nfds_t = if !stdin_eof && free > 0 && rx_cap > 0 {
			1
		} else {
			0
		};
...
fn termio_thread_simple(h: TermIoHandle) {
	...
	while stop.load(Ordering::Acquire) == 0 {
		...
		// TX ring buffer -> hart log files (stdout handled by speedup execution engine directly)
		drain_tx(&h, &mut chunk);
	}
	drain_tx(&h, &mut chunk);
}
