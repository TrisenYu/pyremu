...
impl ModuleState {
    pub fn new(num_harts: u32, active_hart_num: u32) -> Self {
...
pub unsafe extern "C" fn run_parallel(
    ...
    let cc_clint = ConcurrentClintCtx::from_ffi(clint_raw, num_harts);

    // Count active harts once (harts that self-halt will stop their thread).
    let active_hart_num = {
        let mut count: u32 = 0;
    ...
    let module = Arc::new(ModuleState::new(num_harts, active_hart_num));
    ...
    // Build Send-safe wrappers for shared context
    ...
    let virtio_raw: *mut FfiVirtIoCtx = if virtio.is_null() {
        std::ptr::null_mut()
    } else {
        virtio as *mut FfiVirtIoCtx
    };
    ...
    // Per-hart instruction budget: divide max_instrs evenly across harts.
    // With 0 (unlimited), each hart runs until a stop condition.
    ...
    // Spawn threads — each hart gets a disjoint &mut HartState.
    let mut handles = Vec::with_capacity(num_harts as usize);
    for hid in 0..num_harts {
        let mref = Arc::clone(&module);
        let mem_send = shared_mem;
        // 每个 hart 使用自己独立的 64 项 PMP 切片 (cfg/addr 缓冲按
        // hart_id 偏移 hid*64)。绝不跨 hart 共享同一 PMP 缓冲——SMP 启动时
        // 各 hart 的 OpenSBI warm-boot 会并发重写 PMP, 共享缓冲将造成数据竞争
        // 与瞬时执行权限丢失 -> 内核取指访问故障 (cause=1)。
        let pmp_send = if shared_pmp.num > 0 {
            SharedPmpCtx {
                cfg: unsafe { shared_pmp.cfg.add(hid as usize * 64) },
                addr: unsafe { shared_pmp.addr.add(hid as usize * 64) },
                num: shared_pmp.num,
            }
        } else {
            shared_pmp
        ...
        let bp_send: &'static [u64] = unsafe { std::mem::transmute(breakpoints) };
        // Cast to usize to cross thread boundary (disjoint indices guarantee safety)
        let state_addr = unsafe { states.add(hid as usize) } as usize;
        // clint/uart: cast to 'static — Python holds them alive for the FFI call
        let clint_ptr: &'static ConcurrentClintCtx = unsafe { std::mem::transmute(&cc_clint) };
        // Use a static empty UART context when null pointer is passed (tests).
        ...
        let uart_ptr: &'static FfiUartCtx = if uart.is_null() {
            &EMPTY_UART
        } else {
            unsafe { std::mem::transmute(&*uart) }
        };
        let mph = max_per_hart;
        // Raw pointer isn't Send — cast to usize for thread boundary crossing
        let stop_ptr = stop_flag as usize;

        let handle = std::thread::spawn(move || {
            let state = unsafe { &mut *(state_addr as *mut HartState) };
            let stop_flag = stop_ptr as *const u8;
            hart_worker(
                state, hid as u8, mem_send, pmp_send, dev_send, clint_ptr, uart_ptr, &mref,
                bp_send, mph, stop_flag,
            );
        });
        handles.push(handle);
    }
    ...
    // Sum total instructions across all harts
    let mut total: u64 = 0;
    for hid in 0..num_harts {
        let s = unsafe { &*states.add(hid as usize) };
        total = total.wrapping_add(s.total_instrs);
    }
    unsafe {
        (*result).total_instrs = total;
    }

    // Write mtime back through the FFI pointer
    let mtime_val = unsafe { &*cc_clint.mtime }.load(Ordering::SeqCst);
    unsafe {
        *clint_raw.mtime = mtime_val;
    }

    // mtime was already written back above; nothing else to flush.
}
...
mod tests {
    ...
    fn cross_hart_msip_wakes_target() {
        ...
        s1.mtvec = 0x200;                 // M-mode handler at 0x200
    ...
    fn regression_bltz_negative_takes_branch() {
        ...
        unsafe {
            run_parallel(&mut state, 1, 10, &mut result, &mem, &pmp, &clint, &dev,
