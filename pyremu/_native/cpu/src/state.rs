...
pub struct HartState {
    ...
    /// Number of instructions executed by this hart across all batches.
    /// Incremented by ``run_batch`` after each slice; never reset.
...
// ============================================================
//  FFI context structs — group related parameters for run_batch
// ============================================================
...
/// batch exit.  Python reads the buffer after the batch completes.
#[repr(C)]
pub struct FfiUartCtx {
    ...
    /// Updated atomically by Rust; Python reads after batch.
    pub tx_wr: *mut u32,
...
///
/// Rust handles all MMIO register reads/writes inline inside the batch,
/// avoiding expensive exits to Python during the device probe sequence.
/// Only ``QueueNotify`` (offset 0x050) triggers a batch exit — Python
/// then processes the virtqueue descriptors (disk I/O).
///
/// Fields are laid out to match the Python ``VirtIOBlock`` MMIO state
/// 1:1; Python initialises them before the batch and reads back changed
/// fields after the batch.
#[repr(C)]
