...
use crate::handlers::{pmp_ok, try_handle_virtio, PmpCtx, DevCtx, EXIT_SENTINEL};
use crate::state::{HartState, exit_reason};
...
use crate::hart_sched::{read_gpr, ram_read_raw, ram_write_raw, ram_offset, read_ram_cross_page, write_ram_cross_page};
...
impl FpOut {
    ...
    fn fpr(value: u64, fflags: u8) -> Self {
        FpOut { value, fflags, to_gpr: 0, trap: 0 }
    }
    #[inline]
    fn gpr(value: u64, fflags: u8) -> Self {
        FpOut { value, fflags, to_gpr: 1, trap: 0 }
    }
    #[inline]
    fn illegal() -> Self {
        FpOut { value: 0, fflags: 0, to_gpr: 0, trap: 1 }
    }
}
...
fn fmv_x_or_class(funct3: u8, is_double: bool, a: u64) -> FpOut {
    match funct3 {
        ...
        1 => {
            // FCLASS。
            let mask = if is_double { fclass_d(a) } else { fclass_s(unbox_s(a)) };
            FpOut::gpr(mask, 0)
        }
        _ => FpOut::illegal(),
    }
...
pub(crate) fn handle_fp_load_concurrent(
    ...
    let tr = match translate_va(state, ctx, va, false, false) {
        ...
        Err(TranslateFault::AccessFault) => {
            let mut dummy = unsafe { std::mem::zeroed() };
            deliver_trap(state, mcause_val(exc_code::LD_ACCESS_FAULT, false), va, &mut dummy);
            return 0;
        }
    };
    ...
    if let Some(data) = try_handle_virtio(tr.pa, false, 0, size, dev) {
        let boxed = if size == 4 { 0xFFFF_FFFF_0000_0000 | data } else { data };
    ...
    if ram_offset(
        tr.pa, size as u32, ctx.ram_base, ctx.ram_size, ctx.shadow_base, ctx.shadow_size,
    )
    .is_none()
    {
        let mut dummy = unsafe { std::mem::zeroed() };
...
pub(crate) fn handle_fp_store_concurrent(
    ...
    {
        let mut dummy = unsafe { std::mem::zeroed() };
        deliver_trap(state, mcause_val(exc_code::ST_ACCESS_FAULT, false), va, &mut dummy);
        return 0;
    }
