...
pub fn deliver_trap_smode(
    ...
    // consecutive_traps is now tracked only in hart_worker /
    // run_hart_slice (when advance==0 && PC unchanged).  Do NOT
    // increment here — MRET/SRET will zero it on successful
    // completion, and normal sequential instructions reset it too.
    ...
    // Trace traps in ld-linux.so range (PYREMU_TRACE_SRET + diagnostic)
    diag::ld_linux_trap(state, code, tval);
...
pub fn deliver_trap_mmode(
    ...
    // The edge detector in ``sync_msip`` will only re-assert mip.MSIP
    // after CLINT MSIP transitions 0->1 again.
    let is_interrupt = (code >> 63) != 0;
    let exc_code = code & 0x7FFF_FFFF_FFFF_FFFFu64;
    if is_interrupt && exc_code == 3 {
        // MSIP — clear the pending bit (hardware auto-clear)
        state.mip &= !(1u64 << 3);
    }
    ...
    // consecutive_traps is now tracked only in hart_worker /
    // run_hart_slice (when advance==0 && PC unchanged).
    ...
    // Trace traps in ld-linux.so range
    diag::ld_linux_trap(state, code, tval);
...
pub(crate) fn priv_ecall_concurrent(
    ...
    if a7 == 0x54494D45 && a6 == 0 {
        // SBI_TIME set_timer — true fast path
    ...
    deliver_trap(state, ecall_cause, 0, &mut dummy);
...
pub(crate) fn priv_sret_concurrent(
    ...
    state.mode = if spp == 0 {
        // sret -> U-mode: trace register state (gated by PYREMU_TRACE_SRET)
        diag::sret_to_umode(state, _ctx);
        riscv_mode::U
    } else {
        riscv_mode::S
    };
...
mod tests {
    ...
    // FIXME: run_batch path — needs adaptation for concurrent model
    #[ignore]
    #[test]
