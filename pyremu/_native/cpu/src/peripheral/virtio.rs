//! virtio-blk inline MMIO handler — avoids batch exits for register access.
//!
//! Device features, queue descriptors, and config space reads are handled
//! directly in Rust.  Only ``QueueNotify`` sets ``notify_pending=1``,
//! which Python processes at the next natural batch boundary.
