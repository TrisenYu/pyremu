...
from pyremu.platform import PlatformConfig  # noqa: E402
...
def main():
    ...
    def traced_exec(self, instr):
        ...
        if pc == pc_sw_lenp:
            ...
            if state["fdt_driver_init_count"] > 0:
                print(
                    f"[c={emu._cycle:5d}] SW *lenp: *0x{addr:016x} = 0x{val:08x} ({val:>12d})"
                )
        ...
        if pc == pc_lw_proplen1:
            ...
            if data is not None:
                proplen = int.from_bytes(data, "little")
            print(
                f"[c={emu._cycle:5d}] LW proplen(@0x1989C): s0=0x{s0:016x}  "
        ...
        if pc == pc_sw_proplen:
            ...
            proplen_addr = (s0 - 0x6C) & 0xFFFF_FFFF_FFFF_FFFF
            print(
                f"[c={emu._cycle:5d}] SW proplen: s0=0x{s0:016x}  "
    ...
    try:
        ...
        print(
            f"Timeout at cycle {emu._cycle}, pc=0x{e.pc:x}"
            if e.pc
            else f"Timeout at cycle {emu._cycle}"
        )
