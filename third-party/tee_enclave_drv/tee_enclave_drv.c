...
/* ecall with 5 register arguments (a0..a4) + func_id in a6 + ext_id in a7.
 * All arguments use explicit register constraints so the compiler places
 * values in the correct RISC-V ABI registers for the ecall instruction. */
static inline struct sbi_ret sbi_enclave_ecall_5(
    unsigned long func_id,
    unsigned long a0_val, unsigned long a1_val,
    unsigned long a2_val, unsigned long a3_val, unsigned long a4_val)
{
    register unsigned long r_a0 asm("a0") = a0_val;
    register unsigned long r_a1 asm("a1") = a1_val;
    register unsigned long r_a2 asm("a2") = a2_val;
    register unsigned long r_a3 asm("a3") = a3_val;
    register unsigned long r_a4 asm("a4") = a4_val;
    register unsigned long r_a6 asm("a6") = func_id;
    register unsigned long r_a7 asm("a7") = SBI_ENCLAVE_EXT_ID;

    __asm__ volatile(
        "ecall"
        : "+r"(r_a0), "+r"(r_a1)
        : "r"(r_a2), "r"(r_a3), "r"(r_a4), "r"(r_a6), "r"(r_a7)
        : "memory"
    );

    return (struct sbi_ret){.error = r_a0, .value = r_a1};
}

/* ecall with 0 arguments (func_id only). */
static inline struct sbi_ret sbi_enclave_ecall_0(unsigned long func_id)
{
    return sbi_enclave_ecall_5(func_id, 0, 0, 0, 0, 0);
}

/* ecall with 1 argument (func_id + a0). */
static inline struct sbi_ret sbi_enclave_ecall_1(
    unsigned long func_id, unsigned long a0_val)
{
    return sbi_enclave_ecall_5(func_id, a0_val, 0, 0, 0, 0);
}
...
static long tee_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    case TEE_IOC_CREATE: {
        ...
        /* On success, execution transfers to the enclave and does NOT
         * return here.  On failure, a0 carries the error code. */
        if (ret.error != 0) {
            pr_err("tee_enclave: CREATE(400) failed, error=%ld\n", ret.error);
            rc = (long)ret.error ?: -EIO;
        }
        /* CREATE success = enclave running; host thread will receive
         * SUSPEND later.  The enclave_id is in ret.error on success? No —
         * CREATE handler sets a0=enclave_id on success, but out->skip_regs_update=1
         * so the SBI framework doesn't overwrite it.  We return what we got. */
        break;
    }
    ...
    case TEE_IOC_ENTER: {
        ...
        if (kargs.argc > 0 && kargs.argv_ptr != 0) {
            ...
            if (copy_from_user(argv_buf,
                       (void __user *)(unsigned long)kargs.argv_ptr,
                       argv_bytes)) {
                rc = -EFAULT;
                goto out_free_argv;
            }
        ...
        ret = sbi_enclave_ecall_5(SBI_ENCLAVE_ENTER,
                    kargs.enclave_id, kargs.argc,
    ...
    case TEE_IOC_GET_ID: {
        ...
        if (copy_to_user((void __user *)arg, &ret.error,
    ...
    case TEE_IOC_SUSPEND: {
        ...
        /* SUSPEND handler sets a0=0 on success and out->skip_regs_update=1,
         * so mepc is advanced past the ecall by the handler itself. */
        if (ret.error != 0) {
    ...
    case TEE_IOC_RESUME: {
        unsigned long long enclave_id;
        if (copy_from_user(&enclave_id, (void __user *)arg,
                   sizeof(enclave_id)))
            return -EFAULT;
        ret = sbi_enclave_ecall_1(SBI_ENCLAVE_RESUME, enclave_id);
        /* On success, transfers into enclave (does not return). */
        if (ret.error != 0) {
            pr_err("tee_enclave: RESUME(405) id=%llu failed, error=%ld\n",
                   enclave_id, ret.error);
            rc = (long)ret.error ?: -EIO;
        }
        return rc;
    }

    /* ---- SHUTDOWN: destroy current enclave ---- */
    case TEE_IOC_SHUTDOWN: {
        ret = sbi_enclave_ecall_0(SBI_ENCLAVE_SHUTDOWN);
        if (ret.error != 0) {
            pr_err("tee_enclave: SHUTDOWN(403) failed, error=%ld\n",
                   ret.error);
            rc = (long)ret.error ?: -EIO;
        }
        return rc;
