...
static int do_create(int fd, unsigned long long *out_id) {
    ...
    int rc = ioctl(fd, TEE_ENCLAVE_IOC_CREATE, args);
    if (rc < 0) return rc;
    *out_id = args[0];
    return 0;
}

static int do_enter(int fd, unsigned long long enclave_id,
                     const char *payload_path) {
    unsigned long long args[5];
    args[0] = enclave_id;
    args[1] = 0;  // argc
    args[2] = 0;  // argv
    args[3] = 0;  // payload_base_va
    args[4] = 0;  // payload_size
    ...
    if (payload_path) {
        ...
        fread(buf, 1, sz, fp);
        fclose(fp);
        args[3] = (unsigned long long)buf;
        args[4] = (unsigned long long)sz;
    }

    int rc = ioctl(fd, TEE_ENCLAVE_IOC_ENTER, args);
    if (payload_path) free((void *)(unsigned long)args[3]);
    return rc;
}

static void do_shutdown(int fd, unsigned long long enclave_id) {
    unsigned long long args[1] = {enclave_id};
    ioctl(fd, TEE_ENCLAVE_IOC_SHUTDOWN, args);
}

static void child_main(const char *payload_path, int proc_id) {
    int fd = open(TEE_DEVICE_PATH, O_RDWR);
    if (fd < 0) die("open " TEE_DEVICE_PATH);

    // 1. 查询当前 mdid
    unsigned long long mdid = 0;
    int rc = ioctl(fd, TEE_ENCLAVE_IOC_GET_ID, &mdid);
    printf("[child %d pid=%d] GET_ID: mdid=%llu rc=%d\n",
           proc_id, getpid(), (unsigned long long)mdid, rc);
