...
int main(int argc, char **argv)
{
    ...
    printf("=== TEE Enclave Driver Test ===\n\n");
    ...
    rc = do_get_mem(&mem);
    if (rc == 0) {
        printf("[test] GET_MEM: free_total=%llu max_contiguous=%llu (2 MiB units)\n",
    ...
    if (argc < 2) {
        ...
        printf("[test] Skipping CREATE/ENTER — no payload specified.\n");
        printf("[test] Basic ioctl smoke test PASSED.\n");
        close(fd);
        return 0;
    }
    ...
    printf("[test] CREATE: enclave created & suspended, back in host\n");
