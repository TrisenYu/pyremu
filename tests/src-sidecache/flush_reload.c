...
static void flush_addr(uint8_t *probe_base, size_t base_sz, uint8_t *target) {
	uintptr_t t = (uintptr_t)target;
	for (int w = 0; w < EVICT_WAYS; w++) {
		uintptr_t evict = t + (uintptr_t)((w + 1) * SET_STRIDE);
		if (evict >= (uintptr_t)probe_base && evict < (uintptr_t)(probe_base + base_sz)) {
			__asm__ volatile("" ::"r"(*(volatile uint8_t *)evict));
		}
	}
	fence_all();
}
...
static uint64_t time_load(volatile uint8_t *addr) {
	fence_all();
	uint64_t start = rdcycle();
	__asm__ volatile("lb %0, 0(%1)" : "=r"(*(volatile uint8_t *)addr) : "r"(addr));
	fence_all();
...
static uint64_t calibrate(uint8_t *arr, size_t arr_sz) {
	uint64_t hit_sum = 0, miss_sum = 0;

	/* 命中: 反复访问同一行 */
	for (int i = 0; i < CALIBRATE_N; i++) {
		fence_all();
		(void)*(volatile uint8_t *)arr; /* prime */
		fence_all();
...
static uint64_t receiver_probe(uint8_t *arr, int n_bits, uint64_t threshold) {
...
int main(void) {
	puts("=== Flush+Reload Cache Side-Channel ===");
	printf(
		"L2: %d KiB  line: %d B  ways: %d  sets: %d  stride: %d KiB\n",
		L2_KB,
		CACHE_LINE,
		L2_WAYS,
		L2_SETS,
	...
	int ok_count = 0, total_bit_errs = 0;
