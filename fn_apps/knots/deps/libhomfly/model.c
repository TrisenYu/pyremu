...
*/
static void m_recurse(node **model, word *list, weave *oldweave, weave *newweaves) {
	word i, j, wasright, mfirst, msecond, mtop, mbot, list2[BIGWEAVE];
	char mem2[BIGMODEL];
	node *t, **t2, *model2[BIGWEAVE];
	weave other[1];
	Poly temp[1];
	...
	for (t = model[mbot], i = 0; t; t = t->z) {
		i += !t->correct;
	}
	for (t = model[mtop], j = 0; t; t = t->z) {
		j += !t->correct;
	}
	...
	for (j = 0; j < newcross; j++) {
		list2[j] = list[j];
	}
	m_copy(model, model2, mem2);
	*other = *oldweave;
	/* 后续 p_mult 会经 p_init 直接重置 other->tag, 此处深拷贝的结果从未被读取, 反而泄漏,
	 * 故省略; other->boundary 在下方被显式赋值, other->tag 由 p_mult 全新分配。 */
	m_k_switch(t, t->m);
...
static void m_shrink(word *list, node **model, weave *oldweave) {
	Poly blank;
	word this, that, i, j;
	node *t;
...
static void m_switch(word *list, node **model, char *mem) {
	word i, temp[BIGWEAVE], mfirst, msecond;
	...
	if (mfirst != msecond + 1) {
        ...
        */
		...
		for (t2 = model[mfirst]; t2 != 0; t2 = t2->z) {
			t2->self = mfirst;
		}
		for (t2 = model[msecond]; t2 != 0; t2 = t2->z) {
			t2->self = msecond;
		}
	}
...
------------------------------------------------------------------------------
*/
void m_make(word *list, node **model, char *mem) {
	word i, j, temp;
	word offset = 0;
	node *a, *b;
	node *nodemem = (node *)mem;
	...
	for (i = 0; i < oldcross; ++i) {
		model[i] = 0; /* null terminate every string */
	}
