...
void c_handle(
	word *list,		  /* matching of inputs/outputs in thisweave */
	...
	weave *newweaves) /* the array of all new weaves */
{
	...
	for (j = 0, i = 0; i < oldin; ++i, ++j) {
		while (!old_going_in[j]) {
			j++; /* find the next input */
		}
		k		= (i < 6) ? 0 : 1;
...
Poly *c_follow(Instruct *l, int num_crossings) {
	...
	oldweaves->tag.len		 = 0;
	p_term(1, 0, 0, &oldweaves->tag, &oldweaves->tag); /* tag of original link is 1 */
	...
	/* 结果标签原指向 weave 数组内部; 拷贝到独立 Poly 结构后即可释放该数组 */
	{
		Poly *answer = (Poly *)malloc(sizeof(Poly));
