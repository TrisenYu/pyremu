    ...
    It eliminates all terms with coefficients of 0. */
{
	...
	if (inp->len) {
		...
		for (interm = inp->term, outerm = outp->term, endterm = interm + inp->len;
			 interm != endterm;
			 ++interm) {
			if (interm->coef) {
				*(outerm++) = *interm;
...
#define bp_sprintf(...)                                                                  \
	{                                                                                    \
		...
		if (offs > maxlen - bp_chunkreserve) {                                           \
			...
			bp = (char *)realloc(bp, sizeof(char) * maxlen);                             \
		}                                                                                \
	}
	maxlen = bp_chunksize + bp_chunkreserve;
	offs   = 0;
		...
		while (TRUE) {
			...
			} else /* the powers of m and l are equal */
			{
				*(tout)	   = *(t1);
				...
				if (!(tout++)->coef) {
					--tout; /* I don't know if this does anything - TCS */
				}
				if (t1 == end1) {
					break;
				}
...
/*  It is assumed that inp1, inp2, and outp point to existing Poly structs */
{
	...
	xinp2->len	= inp2->len;
	for (t1 = inp1->term, end1 = t1 + inp1->len, end2 = inp2->term + inp2->len; t1 < end1;
		 ++t1) {
