...
static void o_order2(const crossing *k, word *oldorder, word *order, word crossings) {
	word tab[MAXCROSS];
	word j, *ip, *endp, tmp1, tmp2;
	const crossing *kp, *kp2;

	for (ip = tab, endp = tab + crossings; ip != endp;) {
		*(ip++) = 0;
	}
...
/*  This assumes that every crossing is used.
    This lists the crossings in an order my algorithm will hopefully like. */
static void o_order1(const crossing *k, word *oldorder, word *order, word crossings) {
	word tab[MAXCROSS];
	word j, *ip, *endp, tmp1, tmp2;
	const crossing *kp2;
...
static void o_add(
	...
	if (!going_in[i]) {
		while (i && (boundary[--i]->c != newcross || !going_in[i]))
			;
		if (boundary[i]->c == newcross && going_in[i]) {
			old = i;
		}
	}
	answer->which = old;
...
static void o_delete(
	word *n, dllink **boundary, word *going_in, Instruct *answer, word *done, word i) {
	word j, leap = 1;
	dllink *l, *m;
	...
	if ((l == boundary[j]) && (m == boundary[i])) {
		...
		*done						   = 0;
		...
		for (; j < (*n) - 2; j++) {
			boundary[j] = boundary[j + leap];
			going_in[j] = going_in[j + leap];
		}
		answer->reductions++;
		*n -= 2;
	}
...
static void o_one_make(
	...
	for (done = 0; !done;) {
		for (i = *n, done = 1; done && (--i >= 0);) {
			if (*n == 2) {
				break; /* do not shrink to nothing */
			}
...
void o_make(Link *link, Instruct **list) {
	...
	l = (Instruct *)malloc(sizeof(Instruct) * num_crossings);
