...
void b_manip(weave *oldweaves) {
	...
	for (i = 0; !oldweaves[i].tag.len; i++)
		; /* find a defined weave */
	...
	{
		if (first > plan.which + 1) {
			--first;
		}
		if (second > plan.which + 1) {
			--second;
		}
		if (first > plan.which) {
			--first;
		}
		if (second > plan.which) {
			--second;
		}
	} else
		; /* crossing added, no pair removed */

	/*---------------------------------------------------------------- Set map */
	if (plan.reductions == 0) {
		for (i = 0, j = 0; i < oldcross + 2; ++i) {
			if ((i != plan.which) && (i != plan.which + 2)) {
				map[j++] = i;
			}
		}
	} else if (plan.which == (-1)) {
		for (i = 0, j = 0; j < oldcross; ++j) {
			if ((j != plan.r0[0]) && (j != plan.r1[0])) {
				map[j] = i++;
			}
		}
	...
	word *two)	 /* will two new weaves suffice? */
{
	...
	a = plan.which;
	if ((plan.r0[0] == a + 1) || (plan.r1[0] == a + 1)) { /* A Type I Reidemeister move */
		*one = 1;
		return;
	}
	...
	if (plan.r0[0] < plan.r1[0]) {
		if (old_going_in[0] && old_going_in[oldcross - 1]) {
			return;
		}
		...
		if (!old_going_in[first] && !old_going_in[second]
			...
			*two = 1;
			...
			if (plan.which == 0) {
				b_switch(list2, 1, 0, i);
			} else {
				b_switch(list2, newcross - 1, newcross - 2, i);
			}
		} else {
			*one = 1;
		}
